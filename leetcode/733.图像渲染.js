...
var floodFill = function(image, sr, sc, newColor) {
  ...
  const traverse = function(x, y) {
    ...
    image[x][y] = newColor;
    searched[`${x},${y}`] = true;
    if (isValid(image[x-1]) && isValid(image[x-1][y])) { // left
      traverse(x-1, y);
    }
    if (isValid(image[x]) && isValid(image[x][y-1])) { // top
      traverse(x, y-1);
    }
    if (isValid(image[x+1]) && isValid(image[x+1][y])) { // right
      traverse(x+1, y);
    }
    if (isValid(image[x]) && isValid(image[x][y+1])) { // bottom
      traverse(x, y+1);
    }
  }
  traverse(sr, sc);
